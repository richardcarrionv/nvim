...
vim.keymap.set(
	"n",
	"<leader>gd",
	':lua require(\'telescope.builtin\').live_grep({ additional_args = function() return { "--hidden", "--no-ignore" } end })<CR>'
)
vim.keymap.set("n", "<leader>ds", ":lua require('dapui').toggle()<CR>")
...
vim.keymap.set("n", "gn", "<cmd>Telescope lsp_references include_declaration=false show_line=false<CR>", bufopts)
vim.keymap.set(
	"n",
	"gw",
